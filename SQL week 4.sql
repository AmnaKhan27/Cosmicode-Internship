...
SELECT * FROM student
WHERE stu_id IN(
     SELECT stu_id FROM enrollment 
     WHERE c_id = 002);
     
SELECT name FROM stu_marks
WHERE SUM(eng+math+CS)>(
      SELECT AVG(eng+math+CS)/3
      FROM stu_marks);
      
SELECT s.stu_id, s.name, (math + eng + CS)/3 AS avg_marks
FROM student as s
JOIN stu_marks as m ON s.stu_id = m.stu_id
WHERE (m.math + m.eng + m.CS)/3 > (
    SELECT AVG(math + eng + CS)/3
    FROM stu_marks
);
